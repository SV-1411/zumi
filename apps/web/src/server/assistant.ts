import { chatCompletion, openRouterEnabled, type ChatMessage } from './openrouter';
import { enrichLead } from './ai';
...
You MUST reply with a single JSON object, no markdown, with this exact shape:
{
  ...
  "fields": { "name": "", "email": "", "company": "", "projectType": "", "requirements": "", "budget": "", "timeline": "" },
...
Rules:
- Carry forward known fields; only fill new ones. Leave unknown fields as "".
- Ask for name and email naturally (the team needs them to follow up).
- "emotion": "happy" when excited / sharing a recommendation, "confused" when the
  visitor is unclear or you must clarify, "thinking" when weighing options, else "neutral".
- "done": true only once you have name, email and a clear need AND gave a recommendation;
  then "reply" should thank them and say the team will reach out.
- Never invent fields. Keep replies human.`;
...
const SCRIPT: { field: keyof LeadFields; q: string }[] = [
  ...
  { field: 'projectType', q: 'What are you looking to build? (AI agent, SaaS, healthcare, CRM, automation…)' },
...
async function finalize(fields: LeadFields, history: ChatMessage[]): Promise<string | undefined> {
  ...
  try {
    ...
    console.warn('[assistant] finalize failed', err);
    return undefined;
  }
}
...
async function scriptedTurn(history: ChatMessage[], prior: LeadFields): Promise<AssistantTurn> {
  ...
  const next = SCRIPT.find((s) => !fields[s.field]);
  if (next) {
    return {
      reply: next.q.replace('{name}', fields.name ? `, ${fields.name}` : ''),
      fields,
      recommended: null,
      emotion: 'neutral',
      done: false,
...
export async function respond(history: ChatMessage[], prior: LeadFields = {}): Promise<AssistantTurn> {
  if (!openRouterEnabled()) return scriptedTurn(history, prior);

  try {
    const messages: ChatMessage[] = [
      { role: 'system', content: `${SYSTEM_PROMPT}\nKnown fields so far: ${JSON.stringify(prior)}` },
      ...history,
    ];
    ...
    console.warn('[assistant] LLM failed — scripted fallback', err);

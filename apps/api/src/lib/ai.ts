 ...
 * ranges (e.g. "$40k–$60k" -> 60000). Returns the top of any range.
 */
...
/**
 * Server-authoritative lead enrichment. Deterministic heuristics today; this is
 * the seam where a model call (Claude) can produce the scope/summary instead.
 */
export function enrichLead(lead: LeadInput): LeadEnrichment {
  ...
  const { solution, tech } = SOLUTIONS[key];
  ...
  const summary = `${lead.name ?? 'A prospect'} from ${
